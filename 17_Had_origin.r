...
convertlist2str=function(mystring){
  ...
  }else if (grepl("SCH20",mystring)){
    if (grepl("linear",mystring)){
      out = expression("SCH20; linear")
    }else if (grepl("random",mystring)){
      out = expression("SCH20; random")
    }
  }else if (grepl("SOD08-SCH19",mystring)){
    if (grepl("linear",mystring)){
      out = expression("SCH19; linear")
    }else if (grepl("random",mystring)){
      out = expression("SCH19; random")
    }
...
for (icity in as.character(cities)){
  for (iexpid in expids){
    for (iexp in exps){ 
      for (ivar in "Had_Hs"){ # only Had_Hs for plotting...
...
ofile = paste(opath,"/Had_origins_all-cities_",syyyy,"-",eyyyy,"_mean.pdf",sep="")
pdf(ofile, width=7.5, height=4.25, onefile = TRUE, family = "sans", fonts = NULL, version = "1.2", pointsize=16)
# 1x 3-panel plot incl. legend
layout(matrix(1:4,ncol=4),width=c(rep(0.28,3),0.16))
#par(mfrow=c(1,3))
par(mgp=c(1.75,0.5,0))
par(mar=c(5.5,3.5,2.5,0))
par(oma=c(0,0,0,0))
...
for (j in 1:3){ # loop over cities
  ...
  barplot(100*toplot[,myorder],xlab="",ylab="origin [%]",col=cols1, yaxs="i",
          border=NA,offset=0,las=2,yaxt="n",space=0.5,ylim=c(0,110),width=1,
          #legend.text=c("city","land","ocean"),
          #names.arg=mapply(unlist(names(mydata)),FUN=convertlist2str)[myorder],
          cex.names=0.85)
  text(seq(1, 5.5, length.out=ncol(toplot)), par("usr")[3]-1.15, 
  ...
  #box("plot")
  mtext(at=c(-0.15,150),font=2,bty="n",text=paste(letters[j],".",sep=""),outer=FALSE)
  mtext(at=c(1.0,150),font=1,bty="n",text=paste(gdata[[j]]$cname),outer=FALSE,adj=0)
